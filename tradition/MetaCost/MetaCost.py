...
class MetaCost(object):
    ...
    def fit(self, flag, num_class):
        ...
        for i in range(self.m):
            ...
            X = S_[i][col].values
            y = S_[i][flag].values
            M_class.append(sorted(list(Counter(y))))
            print(len(list(Counter(y))))
        ...
        for i in range(len(self.S)):
            ...
            if self.p:
                ...
                for j in range(len(M_)):
                    model = M_[j]
                    # print(M_class[j])
                    temp = model.predict_proba(S_array[[i]])
                    # print(model.predict_proba(S_array[[i]]))
                    # print(temp.shape)
                    list_temp = [0 for num in range(14)]
                    for lab in range(len(M_class[j])):
                        list_temp[int(M_class[j][lab])] = temp.tolist()[0][lab]
                    # print(list_temp)
                    # print(len(list_temp))
                    P_j.append(list_temp)
                #'''
            else:
                P_j = []
                vector = [0] * num_class
